...
configmap=`cat "$DIR/env-configmap.yaml" | sed "s/{{DEV_AWS_BUCKET}}/$AWS_BUCKET/g;s/{{AWS_PROFILE}}/$AWS_PROFILE/g;s/{{DEV_AWS_REGION}}/$AWS_REGION/g;s/{{DEV_POSTGRESS_DB}}/$POSTGRESS_DB/g;s/{{DEV_POSTGRESS_HOST}}/$POSTGRESS_HOST/g;s#{{APP_URL}}#$APP_URL#g"`
echo "$configmap" | kubectl apply -f -

secret=`cat "$DIR/env-secret.yaml" | sed "s/{{JWT_SECRET}}/$JWT_SECRET/g;s/{{DEV_POSTGRESS_USERNAME}}/$POSTGRESS_USERNAME/g;s/{{DEV_POSTGRESS_PASSWORD}}/$POSTGRESS_PASSWORD/g"`
echo "$secret" | kubectl apply -f -

awsSecret=`cat "$DIR/aws-secret.yaml" | sed "s/{{DEV_AWS_CREDENTIALS}}/$AWS_CREDENTIALS/g"`
